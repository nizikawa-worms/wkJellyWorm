#include <stdexcept>
#include <sstream>
#include <lib/hacklib/PatternScanner.h>
#include "Hooks.h"
#include "Config.h"
#include <fstream>
#include <json.hpp>
...
DWORD Hooks::scanPattern(const char *name, const char *pattern, const char* mask, DWORD expected) {
	...
	if(expected && Config::signatureMismatchWarning) {
		uintptr_t rebased = ret - hl::PatternScanner::WAregion.base + 0x401000;
		if(rebased != expected) {
			printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! scanPattern: Possible signature mismatch! Expected: 0x%X Got: 0x%X (0x%X)\n", expected, rebased, ret);
		}
	}
...
void Hooks::saveOffsets(std::string filename) {
	if(!scanFoundNew) return;
	std::ofstream out(filename);
	if (out.good()) {
		nlohmann::json jsonoffsets;
		for(auto & it : scanNameToAddr) {
			jsonoffsets[it.first] = it.second - hl::PatternScanner::WAregion.base + 0x401000;
		}
		out << jsonoffsets.dump(4);
		out.close();
	}
}

void Hooks::loadOffsets(std::string filename) {
	std::ifstream in(filename);
	if (in.good()) {
		std::stringstream buffer;
		buffer << in.rdbuf();
		in.close();
		auto jsonoffsets = nlohmann::json::parse(buffer.str());
		for(auto & it : jsonoffsets.items()) {
			auto addr = (uintptr_t) it.value();
			scanNameToAddr[it.key()] = addr;
		}
	}
}
